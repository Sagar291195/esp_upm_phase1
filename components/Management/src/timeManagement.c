...
void vGetEndDateAndTimeOfSequence(char *endDate, uint8_t sequenceNumber, uint8_t sizeOfEndDate, char *endTime, uint8_t sizeOfEndTimes)
{
    ...
    // ESP_LOGD(TAG,"start year is %d",startTimeTm.tm_year);
    // ESP_LOGD(TAG,"start month is %d",startTimeTm.tm_mon);
    // ESP_LOGD(TAG,"start day is %d",startTimeTm.tm_mday);
    // ESP_LOGD(TAG,"start hour is %d",startTimeTm.tm_hour);
    // ESP_LOGD(TAG,"start min is %d",startTimeTm.tm_min);
    ...
    /**
     * @brief now adding the duration in second to get the new time
     */
    startTimeT = startTimeT + (seq[0].uDurationHour * 3600) + (seq[0].uDurationMinutes * 60);
    ESP_LOGD(TAG, "startTimeT %lu", startTimeT);

    /**
     * @brief converting the time_t format into the struct tm
     */
...
{
    /**
     * @brief getting the pointer to the sequence array
     *
     */

    sequence_t *seq = pGetAddressOfSequenceArray();
    /**
     * @brief iterating over the sequence array to get the total minutes of duration
     *
     */
    ...
    /**
     * @brief converting the minutes into hours and minutes
     */
    ...
    /**
     * @brief converting the hours and minutes into a string
     */
...
void vGetNumberOfHourAndMinutesLeftInStartingSequence(uint16_t *hour, uint8_t *minutes)
{
    ...
    /**
     * @brief subtracting the number of seconds already elapsed  in the running sequnence
     *
     */
    ...
    /**
     * @brief getting the number of hours and minutes left
     *
     */
