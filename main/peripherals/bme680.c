...
esp_err_t bme680_get_measurement_duration(const bme680_t *dev, uint32_t *duration)
{
    ...
    *duration += 1250; // wake up duration from sleep into forced mode
    ...
    *duration += 999; // round up to next ms (1 us ... 1000 us => 1 ms)
    *duration /= 1000;
    *duration += 5; // some ms tolerance
...
esp_err_t bme680_set_filter_size(bme680_t *dev, bme680_filter_size_t size)
{
    ...
    dev->settings.filter_size = size; /* Set the temperature, pressure and humidity settings */
    I2C_DEV_TAKE_MUTEX(&dev->i2c_dev);
    I2C_DEV_CHECK(&dev->i2c_dev, read_reg_8_nolock(dev, BME680_REG_CONFIG, &reg)); // read the current register value
    reg = bme_set_reg_bit(reg, BME680_FILTER, size);                               // set changed bit value
    I2C_DEV_CHECK(&dev->i2c_dev, write_reg_8_nolock(dev, BME680_REG_CONFIG, reg)); // write back the new register value
    I2C_DEV_GIVE_MUTEX(&dev->i2c_dev);
...
esp_err_t bme680_use_heater_profile(bme680_t *dev, int8_t profile)
{
    ...
    uint8_t reg = 0;                                                                             // set
    reg = bme_set_reg_bit(reg, BME680_NB_CONV, profile != BME680_HEATER_NOT_USED ? profile : 0); // set active profile
    reg = bme_set_reg_bit(reg, BME680_RUN_GAS,                                                   // enable or disable gas measurement
...
void get_bme680_pressure_humidity_temp_gas_resi(float *pre, float *temp, float *humi, float *gas_resi)
{
    ...
    bme680_force_measurement(&bme680_sensor);
    ...
    *gas_resi = values.gas_resistance;
}